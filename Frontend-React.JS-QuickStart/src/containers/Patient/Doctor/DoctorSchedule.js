...
class DoctorSchedule extends Component {
    ...
    handleOnechangeSelect = async (event) => {
    ...
    render() {
        let { allDayss, allAvalableTime, isOpenModalBooking, dataScheduleTimeModal } = this.state;
        let { language } = this.props;
        return (
            <>
                <div className='doctor-schedule-container'>
                    <div className='all-shedule'>
                        <select onChange={(event) => this.handleOnechangeSelect(event)}>
                            {allDayss && allDayss.length > 0 &&
                                allDayss.map((item, index) => {
                                    return (
                    ...
                    <div className='all-available-time'>
                        ...
                        <div className='time-content'>
                            {allAvalableTime && allAvalableTime.length > 0 ?
                                        ...
                                        {allAvalableTime.map((item, index) => {
                                            ...
                                            return (
                                                <button key={index} className={language === LANGUAGES.VI ? "btn-vie" : "btn-en"}
                                                    onClick={() => this.handleOnCickScheduletime(item)}
                                                >
                                                    {timeDisplay}
                                                </button>
                                            )
                                ...
                                :
                                <div className='no-shedule'><FormattedMessage id='patient.detail-doctor.no-schedule' /></div>
                            }
                        </div>
                    </div>
                </div>
